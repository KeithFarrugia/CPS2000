...
tokenised_t Lexer::nextToken(std::string src_program_str, int& src_program_idx, const bool is_debug) {
    ...
    while(state != state_t::ST_ERROR){
        ...
        if(is_debug){
            std::cout << "[";
            std::cout << std::setw(7)  << std::left <<  ((next_char == '\n') ? std::string("\\n") : std::string(1, next_char)) + "]";
    ...
    while (!stack.empty()) {
        ...
        if (stack.back() == state_t::ST_SYNTAX_ERROR) { 
            throw std::runtime_error("Error [LEXER]: SYNTAX ERROR AT [" + 
            ((src_program_str[src_program_idx] == '\n') ? std::string("\\n") 
            : std::string(1, src_program_str[src_program_idx])) 
            + "], ID: ["+ std::to_string(src_program_idx) +"]"); 
        }
    ...
    if (token_map.find(state) != token_map.end()) {
        ...
        case  token_t::COLOUR_LITERAL:
            if(!validateColour(temp_tok.lexeme_string)){throw std::runtime_error("Error [LEXER]: INVALID COLOUR LITTERAL");}
            break;
        case token_t::IDENTIFIER:
            temp_tok.token_type = validateKeyword(temp_tok.lexeme_string);
            break;
        case token_t::COMMAND:
            temp_tok.token_type = validateCommand(temp_tok.lexeme_string);
            if(temp_tok.token_type == token_t::COMMAND){throw std::runtime_error("Error [LEXER]: INVALID COMMAND");}
        default:
            break;
        }
        return temp_tok;
